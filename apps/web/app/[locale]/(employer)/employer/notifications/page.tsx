    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="space-y-4">
        {[
          ...
          <div key={i} className={`p-5 rounded-2xl border transition-all cursor-pointer ${n.unread ? 'bg-white border-indigo-100 shadow-lg shadow-indigo-50/50' : 'bg-gray-300/50 border-gray-400'}`}>
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  <h3 className={`text-sm font-black ${n.unread ? 'text-gray-900' : 'text-gray-900'}`}>{n.title}</h3>
                  <span className="text-[10px] font-black text-gray-900 uppercase tracking-tighter">{n.time}</span>
                </div>
                <p className={`text-sm mt-1 leading-relaxed ${n.unread ? 'text-gray-900' : 'text-gray-900'}`}>{n.desc}</p>
              </div>
